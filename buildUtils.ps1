...
function Update-ModuleVersion {
    ...
    $version = Find-Module -Name $ModuleNAme -Repository $RepoName | Select-Object -Expand Version
    $newVersion = Add-VersionObject $version

    Write-Output $newVersion
}

function Add-VersionObject {
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory=$true)]
        [version] $version
    ...
    switch ($true) {
        ($version.build -ge 0) { 
            $output = New-Object Version -ArgumentList $version.major, $version.minor, $($version.build+1)
            break
        }
        ($version.minor -ge 0) { 
            $output = New-Object Version -ArgumentList $version.major, $($version.minor+1)
            break
        }
        Default {
            $output = New-Object Version -ArgumentList $($version.major+1)
        }
    }
    Write-Output $output
}
...
function Publish-SMBModule
{
    ...
    if(Find-Module -Repository $RepoName -Name $ModuleName -ErrorAction SilentlyContinue)
    {
        ...
        $newVersion = Update-ModuleVersion -RepoName $RepoName -ModuleName $ModuleName
    ...
    try
    {
        ...
        Publish-Module -Repository $RepoName -Path $ModulePath
    }
